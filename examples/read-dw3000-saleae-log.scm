...
(define (collect-items reg idxs offset)
  (let loop ((items (register-items reg)) (idxs idxs) (offset offset))
    (cond ((null? items) '())
          ((null? idxs) '())
          ((< (car idxs) offset) (loop items (cdr idxs) offset))
          (else (let* ((it (car items))
                       (ito (item-offset it))
                       (itw (item-width it))
                       (next (+ ito itw))
                       (idx (car idxs)))
                  (if (and (>= idx ito)
                           (< idx next))
                      (cons it (loop (cdr items) (cdr idxs) next))
                      (loop (cdr items) idxs offset)))))))

(define (format-items items)
  (for-each (lambda (item)
              (format #t "  (~a (o ~a) (w ~a) (s ~a))~%"
                      (item-name item)
                      (item-offset item)
                      (item-width item)
                      (semantics-name (item-semantics item))))
            items))

(define (masked-write-decode address mask-size and-mask or-mask)
  (let* ((base-address (car address))
         (sub-address  (cdr address))
         (offset (* 8 sub-address))
         (o+ (lambda (x) (+ x offset)))
         (register (device-ref dw3000 #f base-address))
         (and-idxs (map o+ (bitmask->indices (lognot and-mask) 0 mask-size)))
    ...
    (when (> (length or-idxs) 0)
      (format #t "# Bits set by or-mask ~a:~%" or-idxs)
      (format-items or-items))))
  ...
  (let* ((mosi (assq-ref frame 'mosi))
    ...
    (let ((and-mask (extract-mask mosi 2 mask-size))
      ...
      (format #t
              ...
              (without-time frame))
      (colour-off-newline)
      (masked-write-decode address (* 8 mask-size) and-mask or-mask))))
