...
(define d:item:highlight:name (make-highlighter 'cyan))
...
(define d:combination:highlight:name (make-highlighter 'cyan))
...
(define* (decode-to-text desc #:optional value)
  (when (and (not (device? desc))
             (not value))
    (throw 'missing-value-argument 'decode-to-text))
  (let* ((value* (or value (current-device-state desc)))
         (lst (decode-to-text* desc value*)))
    (for-each (lambda (s)
                (unless (eq? s #:ignore)
                  (display s)
                  (newline)))
