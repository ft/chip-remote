...
(define-immutable-record-type <crp-state>
  (make-crp-state* indentation per-line-prefix port depth level)
  crp-state?
  (indentation     crp:indentation crp:new-indentation)
  (per-line-prefix crp:per-line-prefix)
  (port            crp:port)
  (depth           crp:depth)
  (level           crp:level       crp:new-level))
...
(define* (make-crp-state #:key
  ...
  (make-crp-state* indentation per-line-prefix port depth 0))

(define (new-indent state modification)
  (crp:new-indentation state (+ (crp:indentation state) modification)))

(define (level+ state)
  (crp:new-level state (1+ (crp:level state))))

(define (done? state)
  (and (integer? (crp:depth state))
       (>= (crp:level state)
           (crp:depth state))))
...
(define* (cr-pretty-print obj #:key
                          ...
                          per-line-prefix
                          depth)
  (let ((state (make-crp-state #:port port
...
(define (crp:register state obj)
  (unless (done? state)
    (let ((port (crp:port state))
      ...
      (let ((state* (level+ (new-indent state 2))))
        (unless (done? state*)
      ...
      (let ((state* (level+ (new-indent state 2))))
        (unless (done? state*)
          (for-each (lambda (thing)
                      (let ((idx (car thing))
                            (data (cdr thing))
                            (state** (new-indent state* (+ awidth 4))))
                        (unless (done? state**)
                          (line-combine state*)
                          (if (integer? idx)
                              (format port "~a#x~v,'0x " str:open awidth idx)
                              (format port "~a#f " str:open))
                          (next state** data)
