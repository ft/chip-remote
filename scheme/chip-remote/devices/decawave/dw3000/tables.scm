...
(define gpio-reserved '((reserved . #b011)
                        (reserved . #b100)
                        (reserved . #b101)
                        (reserved . #b110)
                        (reserved . #b111)))

(define-value gpio-0-modes (append '((gpio      . #b000)
                                     (rx-ok-led . #b001)
                                     (pdoa-sw-0 . #b010))
                                   gpio-reserved))

(define-value gpio-1-modes (append '((gpio      . #b00)
                                     (sfd-led   . #b01)
                                     (pdoa-sw-1 . #b10))
                                   gpio-reserved))

(define-value gpio-2-modes (append '((gpio     . #b00)
                                     (rx-led   . #b01)
                                     (pdoa-sw-2 . #b10))
                                   gpio-reserved))

(define-value gpio-3-modes (append '((gpio      . #b00)
                                     (tx-led    . #b01)
                                     (pdoa-sw-3 . #b10))
                                   gpio-reserved))

(define-value gpio-4-modes (append '((gpio        . #b00)
                                     (external-pa . #b01)
                                     (irq         . #b10))
                                   gpio-reserved))

(define-value gpio-5-modes (append '((gpio        . #b00)
                                     (external-tx . #b01)
                                     (reserved    . #b10))
                                   gpio-reserved))

(define-value gpio-6-modes (append '((gpio        . #b00)
                                     (external-rx . #b01)
                                     (reserved    . #b10))
                                   gpio-reserved))

(define-value gpio-7-modes (append '((sync-input . #b00)
                                     (gpio       . #b01)
                                     (reserved   . #b10))
                                   gpio-reserved))

(define-value gpio-8-modes (append '((irq-output . #b00)
                                     (gpio       . #b01)
                                     (reserved   . #b10))
                                   gpio-reserved))
