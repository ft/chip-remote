...
*** Roles
    ...
    /Lines/ are indexed in the following format:
      ...
      - ~<line-name>:<integer>~: ~<line-name>~ is the name specifing the base
        role a line uses. ~<integer>~ is the role index of the specific line.
        Indexing starts at zero. Examples would be ~DATA:0~, ~DATA:1~ etc.; in
        which case ~DATA~ and ~DATA:0~ are equivalent.
...
*** LINE Request
    ...
    within the active mode the line is to be set to.
