...
** Basic Structure
   ...
   A RCCEP system consists of a controlling client and a remote controller that
   possesses one or more ports to which slave devices are attached to it. The
   client talks to the controller using the protocol described herein.
...
** Protocol Basics <<sec:proto-basics>>
   ...
   sign (hey: 0x2b) for implementation defined purposes.
