...
*** PORT Request
    ...
    that may be returned:
      ...
      - "PROPERTIES <CONFIGURABLE|FIXED>": If CONFIGURABLE is returned,
        properties for this port are configurable unless otherwise noted. FIXED
        in turn indicates, that all properties are fixed values and cannot be
        changed.
    ...
    values even if the port itself indicated, it is configurable by returning
    CONFIGURABLE with the PROPERTIES line.

    The request may return other mode-specific properties.
